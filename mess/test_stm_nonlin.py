...
from dynamics.linear_dynamics_LVLH import propagator_absolute,M_to_LVLH,integrate_matrix,matrix_dynamics,propagator_relative
...
def verification(initial_conditions_target_M, initial_conditions_chaser_LVLH, initial_conditions_chaser_M):
    ...
    # ax.plot(-y_orbit[:,0]*r12, -y_orbit[:,1]*r12, y_orbit[:,2]*r12, color='r', label="Chaser's trajectory")
    # ax.scatter(0, 0, 0, label='Moon')
    # ax.scatter((L1x-(1-mu))*r12, 0, 0, label='L1')
    # ax.axis('equal')
    # ax.set_xlabel('X [nd]')
    # ax.set_ylabel('Y [nd]')
    # ax.set_zlabel('Z [nd]')
    ...
    error_distance = np.empty(y_orbit.shape[0])
    error_velocity = np.empty(y_orbit.shape[0])
    for i in range(y_orbit.shape[0]):
        error = la.norm(chaser_trajectory_stm[i,:3] - rho_LVLH_history_true[i,:])
        error_distance[i] = error
        error2 = la.norm(chaser_trajectory_stm[i,3:6] - rho_dot_LVLH_history_true[i,:])
        error_velocity[i] = error2
    
    return error_distance, error_velocity
