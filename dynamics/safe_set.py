...
def passive_safe_ellipsoid(prob, N, inv_Pf, final_time_step):
    """
        N-step Passive safety ellipsoid generation. 
...
def passive_safe_ellipsoid_scvx(prob, state_index):
    ...
    for j in range(N):
        Phi = prob.stm[state_index + N - j - 1,:,:] # TO DO: should there be a -1 in here too? -> think about it
...
def extract_closest_ellipsoid(x_ref, inv_PP_unsafe, l):
    ...
    if inv_PP_unsafe.shape[0] < l:
        ...
        # TO DO: Removing the condition that the state should be outside (ie x>1), see if I need it back
