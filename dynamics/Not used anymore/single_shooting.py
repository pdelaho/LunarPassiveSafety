...
def optimization(initial_conditions, period, μ, max_iter=1000, tol=1e-5, step=3000):
    ...
    for i in range(max_iter):
        y_temp = integrate.odeint(halo_propagator_with_STM, adjusted_conditions, t_simulation, args=(μ,), rtol=1e-12, atol=1e-12)
        ...
        if np.linalg.norm(f) < tol:
            ...
            state_end = halo_propagator(y_temp[-1, :], t_simulation[-1], args=(μ,))
