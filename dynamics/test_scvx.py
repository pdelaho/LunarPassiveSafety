...
sol, log = scvx_star(p_trans, sol_0, μref, fname)
traj = sol["mu"] # in the LVLH frame
print(log["f0"])

# print(sol["mu"][0],p_trans.μ0, sol["mu"][-1], p_trans.μf)
...
for i in range(0,traj.shape[0],10):
    ...
    initial_point = lvlh_to_synodic(traj[i,:],p_trans.target_traj[i],mu)
    t_simulation = p_trans.time_hrz[i:i+p_trans.N_BRS]
    ...
    for j in range(indiv_traj_moon.shape[0]):
        indiv_traj_lvlh[j] = synodic_to_lvlh(indiv_traj_moon[j],p_trans.target_traj[i+j], mu)
...
# fig = plt.figure()
# ax = fig.add_subplot(projection='3d')
# initial_point_moon = lvlh_to_synodic(traj[0,:],p_trans.target_traj[0,:],mu)
# t_simulation = p_trans.time_hrz[:p_trans.n_time]
...
#     traj_lvlh_noControls[i] = synodic_to_lvlh(traj_moon_noControls[i,:],p_trans.target_traj[i,:],mu)
